 ...
 * takes distribution type q value theta_bar and output file on command line
 *************************************************************************
 *                                                               
...
//#include "rngs.h"
//#include "rvgs.h"               /* for random number generation */
#include "random.h"
#ifndef ERROR_INCLUDED
#include "error.h"              /* for fly_team error routine */
#endif
...
void
print_qgt2_visit( double theta_bar, DistParms * DistP ) {
    /* begin print_qgt2 */
    ...
    /* dependent on theta_bar  use alpha 1-17-03 */
    a = DistP->alpha / fact1;
    /* debug 1-13-03 */
    printf( "a = %lf  \n", a );
    /* dependent on theta_bar */
    ...
    //krappa = log( trunc + sqrt( trunc * trunc + 1. / (2. * b * DistP->c)));
    /* debug 1-13-03 */
    printf( "       krappa = %16.40f \n", krappa );
    /* debug 1-13-03 */
    binom_krappa = log( 2 * DistP->trunc + fact3 / ( 4. * DistP->trunc ) );
    /* debug 1-13-03 */
 ...
 */
double
binom_qgt2_visit( double theta_bar, DistParms * DistP ) {       /* begin binom_qgt2_visit */
    /* theta_bar was stariolo's temperature (value 1.) */
    double fact1, fact3, ov_fact3, fact4, fact5;
    ...
    //
    // original code needs more efficient calculations above to keep float
    // exceptions at bay
    ...
    //  area=(a/sqrt(2.*b*DistP->c))*(log(trunc+sqrt(trunc*trunc
    //    +1./(2.*b*DistP->c))) -kappa);
     ...
     ********************************************************************************/
    if( r2 >= g ) {             /* when reject, try again */

        deviate = binom_qgt2_visit( theta_bar, DistP ); /* recursive call */
 ...
 */
double
qgt2_visit( double theta_bar, DistParms * DistP ) {     /* begin qgt2_visit */
    /* theta_bar was stariolo's temperature (value 1.) */
     ...
     *****************************/
    if( r2 >= g ) {             /* when we need to reject, must try again */
        deviate = qgt2_visit( theta_bar, DistP );       /*recursive call */
        /* DistP->rejects = DistP->rejects +1;   may be used for debugging */
    }
    /* endif */
 /*********************************
 ...
 */
double
qlt2_visit( double theta_bar, DistParms * DistP ) {     /* begin qlt2_visit */
     ...
     *****************************/
    if( r2 >= g ) {             /* when we need to reject, must try again */
        deviate = qlt2_visit( theta_bar, DistP );       /*recursive call */
    }
 ...
 */
void
qgt2_init( DistParms * DistP ) {        /* this routine in distributions.c *//* begin qgt2_init */
 ...
 */
void
qlt2_init( DistParms * DistP ) {        /* this routine in distributions.c *//* begin qlt2_init */
    /* these factors are in distributions.h and are static for entire run */
    DistP->gam1 = gammln( 1. / ( DistP->q - 1. ) );
    DistP->gam2 = gammln( 1. / ( DistP->q - 1. ) - 0.5 );
    DistP->fact2 = exp( DistP->gam1 - DistP->gam2 );
 ...
 */
double
generate_dev( double theta_bar, DistParms * DistP ) {   /* begin generate_dev */


    int distribution = DistP->distribution;
    double q = DistP->q;
    ...
    /* end poisson */
    else if( distribution == 7 ) {      /* generalized visiting distribution */
        if( q < 2.0 ) {         /* when q = 2 should use lorentz */
            theta = qlt2_visit( theta_bar, DistP );     /* gsa q between 1 and 2 */
        } else /* less than or equal to 2.6 */ if( q < 2.6 ) {  /* do not use binomial */
            theta = qgt2_visit( theta_bar, DistP );
            /* gsa q between 2 and 2.6 */
        } else {                /* q = 2.6 or more, OK to use binomial */

            theta = binom_qgt2_visit( theta_bar, DistP );
            /* gsa 2.6 <= q < 3 */
        }
    }

    /* end  generalized visiting distribution */
    else if( distribution == 8 ) {      /* gasdev */
        theta = gasdev(  );     /* just try standard normal */
    }


    /* end gasdev returns pos and neg values */
    else if( distribution == 9 ) {      /* pareto */
    ...
    /* end pareto */
    else if( distribution == 10 ) {     /* normal returning negative values  */
        theta = theta_bar * gasdev(  );
                                      /*  normal dist */
    }
